...
\begin{proposition}
    设 $ f \colon X \to Y $, $ S \subseteq X $, 则 $ f(S) \subseteq Y $. 所以对于任意映射 $ f \colon X \to Y $, 恒有值域为陪域的子集: $ f(X) \subseteq Y $.
\end{proposition}
...
\begin{proposition}[\text{有限集上的基数性质}] \label{cardinal}
    ...
    \begin{enumerate}
        ...
        \item (子集-基数) 对于有限集 $ X $, 若 $ X \subseteq Y $, 则 $ Y $ 也是有限集, 且 $ |X| \leqslant |Y| $. 而如果 $ X $ 是 $ Y $ 的真子集, 即 $ X \neq Y $, 则 $ |X| < |Y| $
