...
\begin{definition}[\text{基数}]
    设 $ n $ 为自然数. 集合 $ X $ 的基数为 $ n $, 当且仅当它于集合 $ \set{i \in \N \mid 1 \leqslant i \leqslant n} $ 有相同的基数. $ X $ 具有 $ n $ 个元素, 当且仅当其基数为 $ n $.
\end{definition}
...
\subsection{基数性质}
下面着重讲解有限集上的基数性质.
\begin{proposition}[\text{有限集上基数性质 I}] \label{cardinal I}
    设 $ X $, $ Y $ 均为有限集.
    \begin{enumerate}
...
\begin{figure}[H]
    \centering
    \includegraphics[width = 0.4\linewidth]{implies.pdf}
\end{figure}
...
\begin{proposition}[\text{有限集上的基数性质 II}] \label{cardinal II}
    设 $ X $, $ Y $ 均为有限集.
    \begin{enumerate}
