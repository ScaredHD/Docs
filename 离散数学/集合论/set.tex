...
\begin{proposition}[\text{有限集上的基数性质 I}] \label{cardinal II}
    设 $ X $, $ Y $ 均为有限集.
    \begin{enumerate}
...
\begin{proposition}[\text{有限集上基数性质 II}] \label{cardinal I}
    设 $ X $, $ Y $ 均为有限集.
    \begin{enumerate}
...
\begin{proof}
    (1): $ X \subseteq Y $, 则 $ Y = X \cup (Y \setminus X) $ 以及 $ X \cap (Y \setminus X) = \varnothing $. 于是由有限集上的基数性质 I-(2) $ |Y| = |X \cup (Y \setminus X)| = |X| + |Y \setminus X| \geqslant |X| $.
